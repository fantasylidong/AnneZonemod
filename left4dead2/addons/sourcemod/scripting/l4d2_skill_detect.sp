...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {default}使用近战{blue}满血秒杀{default}正在冲锋的 {olive}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {default}使用近战{blue}满血秒杀{default}正在冲锋的 {olive}牛", attacker );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {default}使用近战{blue}砍死{default}正在冲锋的 {olive}%N {default}({blue}%i 伤害{default})", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {default}使用近战{blue}砍死{default}正在冲锋的 {olive}牛 ({blue}%i 伤害{default})", attacker, damage );
        }
    }
...
stock HandleDeadstop( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}推停了 {olive}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}推停了 {olive}hunter", attacker );
        }
    }
...
HandleTongueCut( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {blue}砍断 {olive}%N{default}的舌头", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{green}★ {olive}%N {blue}空爆 {default}了一个tank石头", attacker );
    }
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)
        ...
        {
            CPrintToChatAll( "{green}★★ {olive}%N {red}高扑了 {olive}%N {default}({red}%i {default}伤害, 高度: {red}%i{default})", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{green}★★ {olive}Hunter {red}高扑了 {olive}%N {default}({red}%i {default}伤害, 高度: {red}%i{default})", victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
    }
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {red}从高出跳下控住 {olive}%N {default}({red}高度{default}: {red}%i{default})", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive} jockey {red}从高处跳下控住 {olive}%N {default}({red}高度{default}: {red}%i{default})", victim, RoundFloat(height) );
        }
    }
...
stock HandleDeathCharge( attacker, victim, Float:height, Float:distance, bool:bCarried = true )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{green}★★★★ {olive}%N {red}对 {olive}%N{default} 完成一次{red}%s死亡冲锋 ({red}高度{default}: {red}%i{default})",
                    attacker,
                    victim,
                    (bCarried) ? "" : "连带",
                    RoundFloat(height)
        ...
        {
            CPrintToChatAll( "{green}★★★★ {olive} charger {red}对 {olive}%N{default} 完成一次{red}%s死亡冲锋 ({red}高度{default}: {red}%i{default})",
                    victim,
                    (bCarried) ? "" : "连带",
                    RoundFloat(height) 
                );
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    CPrintToChatAll( "{green}★ {olive}%N {blue}瞬间将 {olive}%N {default}从 {olive}%N{default}的 %s 手中救下 ({blue}%.2f {default}秒)",
                            attacker, pinVictim, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N {blue}瞬间将 {olive}队友 {default}从 {olive}%N{default}的 %s 手中救下 ({blue}%.2f {default}秒)",
                            attacker, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
