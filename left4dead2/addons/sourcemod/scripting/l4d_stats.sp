...
//	}
	...
	else if (AwardID == 99) // Round restart
	{
		...
		decl String:plugin_name[MAX_LINE_WIDTH];
		GetConVarString(cvar_mode, plugin_name, sizeof(plugin_name));
		if (StrContains(plugin_name, "anne", false)!=-1 || StrContains(plugin_name, "charger", false)!=-1)Score=200;
		else Score=50;
		if (Mode)
...
public CheckSurvivorsWin()
{
	...
	cvar_mode = FindConVar("sv_tags");
	GetConVarString(cvar_mode, plugin_name, sizeof(plugin_name));
	if (StrContains(plugin_name, "1vht", false)!=-1 || StrContains(plugin_name, "alone", false)!=-1)
	{
		StatsPrintToChatTeam(TEAM_SURVIVORS, "\x03单人模式 \x01没有\x04特感击杀奖励，所以过关奖励分数\x053倍");
		Score *=3;
	}
